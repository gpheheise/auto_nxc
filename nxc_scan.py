...
def auth_args(cfg: dict) -> list[str]:
    ...
    if mode == "kcache":
        ...
        return ["-u", cfg["username"], "-p", cfg["password"]]
...
def run_module(nxc_bin: str, label: str, proto: str,
    ...
    try:
        ...
        result.stdout = "[TIMEOUT after 300s]"
        result.returncode = -1
    except Exception as e:
        result.stdout = f"[ERROR: {e}]"
        result.returncode = -1
    ...
    status = f"{G}✓ success{RST}" if result.success else f"{Y}· ran{RST}"
    if result.pwned:
        status = f"{R}★ PWNED{RST}"
    with _print_lock:
        print(f"  {B}↳{RST} {out_file.name}  [{result.duration_s}s]  {status}")
    ...
    with ThreadPoolExecutor(max_workers=max_workers) as pool:
        ...
        for future in as_completed(futures):
            try:
                ...
                warn(f"Module {futures[future]} raised: {e}")
